...
@implementation XDTAFSAMKeychainQuery
...
- (BOOL)save:(NSError *__autoreleasing *)error {
	OSStatus status = XDTAFSAMKeychainErrorBadArguments;
	...
	if (status == errSecSuccess) {//item already exists, update it!
		...
		CFTypeRef accessibilityType = [XDTAFSAMKeychain accessibilityType];
...
- (NSMutableDictionary *)query {
	...
	if ([[self class] isSynchronizationAvailable]) {
		...
		switch (self.synchronizationMode) {
			case XDTAFSAMKeychainQuerySynchronizationModeNo: {
			  value = @NO;
			  break;
			}
			case XDTAFSAMKeychainQuerySynchronizationModeYes: {
			  value = @YES;
			  break;
			}
			case XDTAFSAMKeychainQuerySynchronizationModeAny: {
...
+ (NSError *)errorWithCode:(OSStatus) code {
	...
	dispatch_once(&onceToken, ^{
		NSURL *url = [[NSBundle bundleForClass:[XDTAFSAMKeychainQuery class]] URLForResource:@"SAMKeychain" withExtension:@"bundle"];
	...
	switch (code) {
		...
		case XDTAFSAMKeychainErrorBadArguments: message = NSLocalizedStringFromTableInBundle(@"XDTAFSAMKeychainErrorBadArguments", @"SAMKeychain", resourcesBundle, nil); break;
	...
	return [NSError errorWithDomain:kXDTAFSAMKeychainErrorDomain code:code userInfo:userInfo];
}
