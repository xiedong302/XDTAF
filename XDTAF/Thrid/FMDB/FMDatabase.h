...
NS_ASSUME_NONNULL_BEGIN
    ...
    #define XDTAFFMDBAutorelease(__v) ([__v autorelease]);
    #define XDTAFFMDBReturnAutoreleased XDTAFFMDBAutorelease

    #define XDTAFFMDBRetain(__v) ([__v retain]);
    #define XDTAFFMDBReturnRetained XDTAFFMDBRetain

    #define XDTAFFMDBRelease(__v) ([__v release]);

    #define XDTAFFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define XDTAFFMDBAutorelease(__v)
    #define XDTAFFMDBReturnAutoreleased(__v) (__v)

    #define XDTAFFMDBRetain(__v)
    #define XDTAFFMDBReturnRetained(__v) (__v)

    #define XDTAFFMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define XDTAFFMDBDispatchQueueRelease(__v)
    #else
        #define XDTAFFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^XDTAFFMDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);

typedef NS_ENUM(int, XDTAFFMDBCheckpointMode) {
    XDTAFFMDBCheckpointModePassive  = 0, // SQLITE_CHECKPOINT_PASSIVE,
    XDTAFFMDBCheckpointModeFull     = 1, // SQLITE_CHECKPOINT_FULL,
    XDTAFFMDBCheckpointModeRestart  = 2, // SQLITE_CHECKPOINT_RESTART,
    XDTAFFMDBCheckpointModeTruncate = 3  // SQLITE_CHECKPOINT_TRUNCATE
};
...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 - `XDTAFFMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<XDTAFXDTAFFMResultSet>` - Represents the results of executing a query on an `XDTAFFMDatabase`.
 - `<XDTAFFMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<XDTAFFMDatabasePool>` - A pool of `XDTAFFMDatabase` objects.
 - `<XDTAFFMStatement>` - A wrapper for `sqlite_stmt`.
 ...
 @warning Do not instantiate a single `XDTAFFMDatabase` object and use it across multiple threads. Instead, use `<XDTAFFMDatabaseQueue>`.
...
@interface XDTAFFMDatabase : NSObject
...
/** Create a `XDTAFFMDatabase` object.
 
 An `XDTAFFMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `XDTAFFMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `XDTAFFMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    XDTAFFMDatabase *db = [XDTAFFMDatabase databaseWithPath:@"/tmp/tmp.db"];
 ...
 @return `XDTAFFMDatabase` object if successful; `nil` if failure.
...
/** Create a `XDTAFFMDatabase` object.
 ...
 2. `nil`.  An in-memory database is created.  This database will be destroyed with the `XDTAFFMDatabase` connection is closed.
...
/** Initialize a `XDTAFFMDatabase` object.
...
/** Initialize a `XDTAFFMDatabase` object.
 ...
 An `XDTAFFMDatabase` is created with a local file URL to a SQLite database file.  This path can be one of these three:
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) XDTAFFMDBExecuteStatementsCallbackBlock _Nullable)block;
...
/** Execute select statement
 ...
 Executing queries returns an `<XDTAFFMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[XDTAFFMResultSet next]>`) from one record to the other.
 ...
 @return A `<XDTAFFMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see XDTAFFMResultSet
 @see [`XDTAFFMResultSet next`](<[XDTAFFMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
...
- (XDTAFFMResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (XDTAFFMResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (XDTAFFMResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> XDTAFFMResultSet!`
...
- (XDTAFFMResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (XDTAFFMResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (XDTAFFMResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;
...
/** Begin a transaction
 ...
 @warning    Unlike SQLite's `BEGIN TRANSACTION`, this method currently performs
             ...
             is likely to change in future versions of XDTAFFMDB, whereby this method
 ...
 */
- (BOOL)checkpoint:(XDTAFFMDBCheckpointMode)checkpointMode error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return YES on success, otherwise NO.
 */
- (BOOL)checkpoint:(XDTAFFMDBCheckpointMode)checkpointMode name:(NSString * _Nullable)name error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return YES on success, otherwise NO.
 */
- (BOOL)checkpoint:(XDTAFFMDBCheckpointMode)checkpointMode name:(NSString * _Nullable)name logFrameCount:(int * _Nullable)logFrameCount checkpointCount:(int * _Nullable)checkpointCount error:(NSError * _Nullable *)error;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    XDTAFFMResultSet *rs = [db executeQuery:@"SELECT * FROM employees WHERE RemoveDiacritics(first_name) LIKE 'jose'"];
...
typedef NS_ENUM(int, XDTAFSqliteValueType) {
    XDTAFSqliteValueTypeInteger = 1,
    XDTAFSqliteValueTypeFloat   = 2,
    XDTAFSqliteValueTypeText    = 3,
    XDTAFSqliteValueTypeBlob    = 4,
    XDTAFSqliteValueTypeNull    = 5
};

- (XDTAFSqliteValueType)valueType:(void *)argv;
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [XDTAFFMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using XDTAFFMDatabase::storeableDateFormat.
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using FMDB you will not need to interact directly with `XDTAFFMStatement`, but rather with `<XDTAFFMDatabase>` and `<XDTAFXDTAFFMResultSet>` only.
 
 ### See also
 
 - `<XDTAFFMDatabase>`
 - `<XDTAFXDTAFFMResultSet>`
 - [`sqlite3_stmt`](http://www.sqlite.org/c3ref/stmt.html)
 */

@interface XDTAFFMStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
