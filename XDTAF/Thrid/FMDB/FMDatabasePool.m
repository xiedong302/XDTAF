...
- (void)pushDatabaseBackInPool:(XDTAFFMDatabase*)db;
- (XDTAFFMDatabase*)db;

@end


@implementation XDTAFFMDatabasePool
...
+ (instancetype)databasePoolWithURL:(NSURL *)url {
    return XDTAFFMDBReturnAutoreleased([[self alloc] initWithPath:url.path]);
}

+ (instancetype)databasePoolWithPath:(NSString *)aPath flags:(int)openFlags {
    return XDTAFFMDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}

+ (instancetype)databasePoolWithURL:(NSURL *)url flags:(int)openFlags {
    return XDTAFFMDBReturnAutoreleased([[self alloc] initWithPath:url.path flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags vfs:(NSString *)vfsName {
    ...
    if (self != nil) {
        _path               = [aPath copy];
        _lockQueue          = dispatch_queue_create([[NSString stringWithFormat:@"upchina_taf_fmdb.%@", self] UTF8String], NULL);
        _databaseInPool     = XDTAFFMDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = XDTAFFMDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    XDTAFFMDBRelease(_databaseInPool);
    XDTAFFMDBRelease(_databaseOutPool);
    XDTAFFMDBRelease(_vfsName);
    
    if (_lockQueue) {
        XDTAFFMDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)pushDatabaseBackInPool:(XDTAFFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The XDTAFFMDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
...
#endif
        if (success) {
            if ([self->_delegate respondsToSelector:@selector(upDatabasePool:shouldAddDatabaseToPool:)] && ![self->_delegate upDatabasePool:self shouldAddDatabaseToPool:db]) {
                [db close];
                db = 0x00;
            }
            else {
                ...
                if (![self->_databaseOutPool containsObject:db]) {
                    ...
                    if (shouldNotifyDelegate && [self->_delegate respondsToSelector:@selector(upDatabasePool:didAddDatabase:)]) {
                        [self->_delegate upDatabasePool:self didAddDatabase:db];
                    }
                }
            }
        }
