...
@interface XDTAFFMResultSet () {
    NSMutableDictionary *_columnNameToIndexMap;
}
@end

@implementation XDTAFFMResultSet

+ (instancetype)resultSetWithStatement:(XDTAFFMStatement *)statement usingParentDatabase:(XDTAFFMDatabase*)aDB {
    
    XDTAFFMResultSet *rs = [[XDTAFFMResultSet alloc] init];
    ...
    return XDTAFFMDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    XDTAFFMDBRelease(_columnNameToIndexMap);
...
- (NSDictionary *)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return XDTAFFMDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"XDTAFFMDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
