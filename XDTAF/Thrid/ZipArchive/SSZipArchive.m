...
NSString *const XDTAFSSZipArchiveErrorDomain = @"XDTAFSSZipArchiveErrorDomain";

#define CHUNK 16384

static int _zipOpenEntry(zipFile entry, NSString *name, const zip_fileinfo *zipfi, int level, NSString *password, BOOL aes);
static BOOL _fileIsSymbolicLink(const unz_file_info *fileInfo);
...
@interface NSData(XDTAFSSZipArchive)
- (NSString *)_xdtaf_base64RFC4648 API_AVAILABLE(macos(10.9), ios(7.0), watchos(2.0), tvos(9.0));
- (NSString *)_xdtaf_hexString;
@end

@interface NSString (XDTAFSSZipArchive)
- (NSString *)_xdtaf_sanitizedPath;
@end

@interface XDTAFSSZipArchive ()
- (instancetype)init NS_DESIGNATED_INITIALIZER;
@end

@implementation XDTAFSSZipArchive
{
    /// path for zip file
    NSString *_path;
    zipFile _zip;
...
+ (BOOL)isFilePasswordProtectedAtPath:(NSString *)path {
    // Begin opening
    zipFile zip = xdtaf_unzOpen(path.fileSystemRepresentation);
    if (zip == NULL) {
        return NO;
    }
    
    BOOL passwordProtected = NO;
    int ret = xdtaf_unzGoToFirstFile(zip);
    if (ret == UNZ_OK) {
        do {
            ret = xdtaf_unzOpenCurrentFile(zip);
            if (ret != UNZ_OK) {
                // attempting with an arbitrary password to workaround `unzOpenCurrentFile` limitation on AES encrypted files
                ret = xdtaf_unzOpenCurrentFilePassword(zip, "");
                xdtaf_unzCloseCurrentFile(zip);
            ...
            ret = xdtaf_unzGetCurrentFileInfo(zip, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
            xdtaf_unzCloseCurrentFile(zip);
            if (ret != UNZ_OK) {
                break;
...
+ (BOOL)isPasswordValidForArchiveAtPath:(NSString *)path password:(NSString *)pw error:(NSError **)error {
    ...
    if (zip == NULL) {
        if (error) {
            *error = [NSError errorWithDomain:XDTAFSSZipArchiveErrorDomain
                                         code:XDTAFSSZipArchiveErrorCodeFailedOpenZipFile
    ...
    if (ret == UNZ_OK) {
        do {
            if (pw.length == 0) {
                ...
                ret = xdtaf_unzOpenCurrentFilePassword(zip, [pw cStringUsingEncoding:NSUTF8StringEncoding]);
            }
            if (ret != UNZ_OK) {
                if (ret != MZ_PASSWORD_ERROR) {
                    if (error) {
                        *error = [NSError errorWithDomain:XDTAFSSZipArchiveErrorDomain
                                                     code:XDTAFSSZipArchiveErrorCodeFailedOpenFileInZip
            ...
            if (ret != UNZ_OK) {
                if (error) {
                    *error = [NSError errorWithDomain:XDTAFSSZipArchiveErrorDomain
                                                 code:XDTAFSSZipArchiveErrorCodeFileInfoNotLoadable
                ...
                int readBytes = xdtaf_unzReadCurrentFile(zip, buffer, (unsigned)MIN(10UL,fileInfo.uncompressed_size));
                if (readBytes < 0) {
                    ...
                    if (readBytes != Z_DATA_ERROR) {
                        if (error) {
                            *error = [NSError errorWithDomain:XDTAFSSZipArchiveErrorDomain
                                                         code:XDTAFSSZipArchiveErrorCodeFileContentNotReadable
...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination delegate:(nullable id<XDTAFSSZipArchiveDelegate>)delegate
{
          ...
          toDestination:(NSString *)destination
              overwrite:(BOOL)overwrite
               password:(nullable NSString *)password
                  error:(NSError **)error
               delegate:(nullable id<XDTAFSSZipArchiveDelegate>)delegate
{
...
+ (BOOL)unzipFileAtPath:(NSString *)path
        ...
        progressHandler:(void (^)(NSString *entry, unz_file_info zipInfo, long entryNumber, long total, BOOL *canceled))progressHandler
...
+ (BOOL)unzipFileAtPath:(NSString *)path
          toDestination:(NSString *)destination
        progressHandler:(void (^_Nullable)(NSString *entry, unz_file_info zipInfo, long entryNumber, long total, BOOL *canceled))progressHandler
      ...
      completionHandler:(void (^_Nullable)(NSString *path, BOOL succeeded, NSError * _Nullable error))completionHandler
{
    ...
    if (path.length == 0 || destination.length == 0)
    {
        ...
        NSError *err = [NSError errorWithDomain:XDTAFSSZipArchiveErrorDomain code:XDTAFSSZipArchiveErrorCodeInvalidArguments userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    if (zip == NULL)
    {
        ...
        NSError *err = [NSError errorWithDomain:XDTAFSSZipArchiveErrorDomain code:XDTAFSSZipArchiveErrorCodeFailedOpenZipFile userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    xdtaf_unzGetGlobalInfo(zip, &globalInfo);
    
    // Begin unzipping
    int ret = 0;
    ret = xdtaf_unzGoToFirstFile(zip);
    if (ret != UNZ_OK && ret != MZ_END_OF_LIST)
    {
        NSDictionary *userInfo = @{NSLocalizedDescriptionKey: @"failed to open first file in zip file"};
        NSError *err = [NSError errorWithDomain:XDTAFSSZipArchiveErrorDomain code:XDTAFSSZipArchiveErrorCodeFailedOpenFileInZip userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    do {
        ...
        @autoreleasepool {
            if (password.length == 0) {
                ...
                ret = xdtaf_unzOpenCurrentFilePassword(zip, [password cStringUsingEncoding:NSUTF8StringEncoding]);
            }
            
            if (ret != UNZ_OK) {
                unzippingError = [NSError errorWithDomain:XDTAFSSZipArchiveErrorDomain code:XDTAFSSZipArchiveErrorCodeFailedOpenFileInZip userInfo:@{NSLocalizedDescriptionKey: @"failed to open file in zip file"}];
                success = NO;
                break;
            }
            ...
            ret = xdtaf_unzGetCurrentFileInfo(zip, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
            if (ret != UNZ_OK) {
                unzippingError = [NSError errorWithDomain:XDTAFSSZipArchiveErrorDomain code:XDTAFSSZipArchiveErrorCodeFileInfoNotLoadable userInfo:@{NSLocalizedDescriptionKey: @"failed to retrieve info for file"}];
                success = NO;
                xdtaf_unzCloseCurrentFile(zip);
                break;
            }
            ...
            xdtaf_unzGetCurrentFileInfo(zip, &fileInfo, filename, fileInfo.size_filename + 1, NULL, 0, NULL, 0);
            filename[fileInfo.size_filename] = '\0';
            
            BOOL fileIsSymbolicLink = _fileIsSymbolicLink(&fileInfo);
            
            NSString * strPath = [XDTAFSSZipArchive _filenameStringWithCString:filename
            ...
            strPath = [strPath _xdtaf_sanitizedPath];
            if (!strPath.length) {
            ...
            if (isDirectory && !fileIsSymbolicLink) {
                ...
                int readBytes = xdtaf_unzReadCurrentFile(zip, buffer, 4096);
                if (readBytes >= 0) {
                    ...
                    while (fp) {
                        if (readBytes > 0) {
                            if (0 == fwrite(buffer, readBytes, 1, fp)) {
                                if (ferror(fp)) {
                                    ...
                                    success = NO;
                                    unzippingError = [NSError errorWithDomain:XDTAFSSZipArchiveErrorDomain code:XDTAFSSZipArchiveErrorCodeFailedToWriteFile userInfo:@{NSLocalizedDescriptionKey: message}];
                                    break;
                                }
                            }
                        } else {
                        ...
                        readBytes = xdtaf_unzReadCurrentFile(zip, buffer, 4096);
                        if (readBytes < 0) {
                            // Let's assume error Z_DATA_ERROR is caused by an invalid password
                            // Let's assume other errors are caused by Content Not Readable
                            success = NO;
            ...
            else
            {
                ...
                int bytesRead = 0;
                while ((bytesRead = xdtaf_unzReadCurrentFile(zip, buffer, 4096)) > 0)
                {
                    buffer[bytesRead] = 0;
                ...
                if (overwrite)
                {
                    if ([fileManager fileExistsAtPath:fullPath])
                    {
                        ...
                        if (!removeSuccess)
                        {
                            ...
                            NSLog(@"[SSZipArchive] %@", message);
                            success = NO;
                            unzippingError = [NSError errorWithDomain:XDTAFSSZipArchiveErrorDomain code:localError.code userInfo:@{NSLocalizedDescriptionKey: message}];
                        }
                    }
                }
            ...
            crc_ret = xdtaf_unzCloseCurrentFile(zip);
            ...
            if (progressHandler)
            {
                progressHandler(strPath, fileInfo, currentFileNumber, globalInfo.number_entry, &canceled);
                if(canceled) break;
            }
        }
    ...
    if (crc_ret == MZ_CRC_ERROR)
    {
        ...
        retErr = [NSError errorWithDomain:XDTAFSSZipArchiveErrorDomain code:XDTAFSSZipArchiveErrorCodeFileInfoNotLoadable userInfo:userInfo];
    }
...
{
    return [XDTAFSSZipArchive createZipFileAtPath:path withFilesAtPaths:paths withPassword:nil];
}
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath {
    return [XDTAFSSZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath withPassword:nil];
}

+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirectory {
    return [XDTAFSSZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:keepParentDirectory withPassword:nil];
}

+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray<NSString *> *)paths withPassword:(NSString *)password
{
    XDTAFSSZipArchive *zipArchive = [[XDTAFSSZipArchive alloc] initWithPath:path];
...
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath withPassword:(nullable NSString *)password {
    return [XDTAFSSZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:NO withPassword:password];
}


+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirectory withPassword:(nullable NSString *)password {
    return [XDTAFSSZipArchive createZipFileAtPath:path
...
- (BOOL)open
{
    ...
    _zip = xdtaf_zipOpen(_path.fileSystemRepresentation, APPEND_STATUS_CREATE);
    return (NULL != _zip);
}
...
- (BOOL)writeFolderAtPath:(NSString *)path withFolderName:(NSString *)folderName withPassword:(nullable NSString *)password
{
    ...
    [XDTAFSSZipArchive zipInfo:&zipInfo setAttributesOfItemAtPath:path];
    
    int error = _zipOpenEntry(_zip, [folderName stringByAppendingString:@"/"], &zipInfo, Z_NO_COMPRESSION, password, NO);
    const void *buffer = NULL;
    xdtaf_zipWriteInFileInZip(_zip, buffer, 0);
    xdtaf_zipCloseFileInZip(_zip);
    return error == ZIP_OK;
}
...
- (BOOL)writeFileAtPath:(NSString *)path withFileName:(nullable NSString *)fileName compressionLevel:(int)compressionLevel password:(nullable NSString *)password AES:(BOOL)aes
{
    ...
    while (!feof(input) && !ferror(input))
    {
        ...
        xdtaf_zipWriteInFileInZip(_zip, buffer, len);
    }
...
- (BOOL)writeData:(NSData *)data filename:(nullable NSString *)filename compressionLevel:(int)compressionLevel password:(nullable NSString *)password AES:(BOOL)aes
{
    ...
    zip_fileinfo zipInfo = {};
    [XDTAFSSZipArchive zipInfo:&zipInfo setDate:[NSDate date]];
    
    int error = _zipOpenEntry(_zip, filename, &zipInfo, compressionLevel, password, aes);
    
    xdtaf_zipWriteInFileInZip(_zip, data.bytes, (unsigned int)data.length);
...
- (BOOL)close
{
    ...
    int error = xdtaf_zipClose(_zip, NULL);
    _zip = nil;
    return error == ZIP_OK;
}
...
{
    NSCalendar *currentCalendar = XDTAFSSZipArchive._gregorian;
...
int _zipOpenEntry(zipFile entry, NSString *name, const zip_fileinfo *zipfi, int level, NSString *password, BOOL aes)
{
    ...
    return xdtaf_zipOpenNewFileInZip5(entry, name.fileSystemRepresentation, zipfi, NULL, 0, NULL, 0, NULL, Z_DEFLATED, level, 0, -MAX_WBITS, DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY, password.UTF8String, aes, made_on_darwin, flag_base, 0);
}
...
@implementation NSData (XDTAFSSZipArchive)
...
@implementation NSString (XDTAFSSZipArchive)
