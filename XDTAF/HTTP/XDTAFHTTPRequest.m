...
- (instancetype)init {
    self = [super init];
    if (self) {
        _formDict = [NSMutableDictionary dictionaryWithCapacity:4];
    }
    return self;
}
...
- (void)add:(NSString *)name value:(NSString *)value {
    if (name && name.length && value && value.length) {
        [self.formDict setObject:XDTAFHTTPEncode(value) forKey:XDTAFHTTPEncode(name)];
    }
}
...
- (instancetype)initWith:(NSString *)URLString method:(NSString *)method body:(id<XDTAFHTTPRequestBody>)body timeout:(NSTimeInterval)timeout {
    self = [super init];
...
+ (instancetype)get:(NSString *)URLString {
    return [XDTAFHTTPRequest get:URLString timeout:30];
}
...
+ (instancetype)post:(NSString *)URLString body:(id<XDTAFHTTPRequestBody>)body {
    return [XDTAFHTTPRequest post:URLString body:body timeout:30];
}
...
- (void)addHeader:(NSString *)name value:(NSString *)value {
    [self.header add:name value:value];
}

- (void)addEncodedHeader:(NSString *)name value:(NSString *)value {
    [self.header addEncoded:name value:value];
}
